...
class SqliteTaskRepository(TaskRepository):
    ...
    def list_tasks(self) -> list[Task]:
        ...
        Returns:
            List of root Task instances (parent_task_id IS NULL)
        """
        try:
            ...
            # Load all tasks
            cursor = conn.execute("""
                SELECT id, parent_task_id, order_num, title, status, priority,
                       due_date, link_url, link_text, repeat_json,
            ...
            for task_id, task in tasks_by_id.items():
                ...
                if parent_id is None:
                    ...
                    if parent:
                        ...
                        _log.warning("Orphaned subtask %s (parent %s not found)", task_id, parent_id)
    ...
    def _upsert_task_recursive(self, conn, task: Task, parent_id: UUID | None) -> None:
        ...
        # Prepare data
        repeat_json = json.dumps(task.repeat) if task.repeat else None
        
        # Upsert task
        conn.execute("""
            INSERT OR REPLACE INTO tasks (
                id, parent_task_id, order_num, title, status, priority,
                due_date, link_url, link_text, repeat_json,
                completed_at, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
    ...
    def _row_to_task(self, task_row: tuple, notes_rows: list[tuple]) -> Task:
        ...
        # Parse repeat_json
        repeat = None
        if task_row[9]:  # repeat_json
            try:
                repeat = json.loads(task_row[9])
            except json.JSONDecodeError as e:
                _log.warning("Failed to parse repeat_json for task %s: %s", task_row[0], e)
