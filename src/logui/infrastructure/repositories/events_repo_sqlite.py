...
class SqliteEventRepository(EventRepository):
    ...
    def list_events(self) -> list[Event]:
        ...
        try:
            ...
            # Load all events
            cursor = conn.execute("""
                SELECT id, title, event_date, start_time, end_time, end_day_offset,
                       notify, notify_minutes_before, repeat_data,
    ...
    def upsert_event(self, event: Event) -> None:
        ...
        with self._db.transaction() as conn:
            ...
            # Upsert event
            repeat_data = json.dumps(event.repeat) if event.repeat else None

            conn.execute(
                """
                INSERT OR REPLACE INTO events (
                    id, title, event_date, start_time, end_time, end_day_offset,
                    notify, notify_minutes_before, repeat_data,
                    created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
    ...
    def _row_to_event(self, event_row: tuple, notes_rows: list[tuple]) -> Event:
        ...
        # Parse repeat_data
        repeat = None
        if event_row[8]:  # repeat_data
            try:
                repeat = json.loads(event_row[8])
            except json.JSONDecodeError as e:
                _log.warning("Failed to parse repeat_data for event %s: %s", event_row[0], e)
