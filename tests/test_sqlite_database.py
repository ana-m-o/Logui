...
def test_sqlite_database_transaction_commit(tmp_path: Path) -> None:
    ...
    with db.transaction() as conn:
        conn.execute("""
            INSERT INTO journal_entries (entry_date, text, updated_at)
            VALUES ('2026-01-01', 'Test entry', '2026-01-01T10:00:00Z')
        """)

    # Verify data was committed
    conn = db.get_connection()
    cursor = conn.execute("SELECT text FROM journal_entries WHERE entry_date = '2026-01-01'")
    row = cursor.fetchone()
...
def test_sqlite_database_foreign_key_cascade_delete(tmp_path: Path) -> None:
    ...
    with db.transaction() as conn:
        ...
        conn.execute("""
            INSERT INTO events (
                id, title, event_date, end_day_offset, notify,
...
def test_sqlite_database_check_constraint_end_time_requires_start_time(tmp_path: Path) -> None:
    ...
    with pytest.raises(sqlite3.IntegrityError):
        with db.transaction() as conn:
            conn.execute("""
                INSERT INTO events (
                    id, title, event_date, start_time, end_time, end_day_offset, notify,
                    created_at, updated_at
                ) VALUES (
...
def test_sqlite_database_check_constraint_task_status(tmp_path: Path) -> None:
    ...
    with pytest.raises(sqlite3.IntegrityError):
        with db.transaction() as conn:
            conn.execute("""
                INSERT INTO tasks (
                    id, task_order, title, status, priority,
                    created_at, updated_at
                ) VALUES (
